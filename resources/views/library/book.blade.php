...
@section('breadcrumb')
    <div id="breadcrumb">
        ...
        <span>Library</span>
        <a href="{{ route("book", ["slug"=> $book->slug]) }}" class="nes-text is-primary">{{ Str::limit($book->title, 50) }}</a>
    </div>
@endsection
                ...
                <div class="book-main-data">
                    ...
                    @if(count($book->authors))
                        <div class="authors">
                            ...
                            <a href="{{ route("books.by_author", ["author"=> $author]) }}"
                                ...
                                <span>
                                        ...
                                        @if($author->birth_year or $author->death_year)
                                            <i class="birth-death-years">
                                                ...
                                                -
                                                @if($author->death_year)<span itemprop="deathDate">{{ $author->death_year }}</span>@else?@endif)</i>
                                        @endif
                                    </span>
                            </a>
                            @endforeach
